...
void CountryLinkedList::deleteNodeWithName(char *name) {
    if (isEmpty()) {
        return;
    }

    CountryLinkedListNode *current = head;
    CountryLinkedListNode *previous = NULL;

    while (strcmp(current->getCountry()->getName(), name) != 0) {
